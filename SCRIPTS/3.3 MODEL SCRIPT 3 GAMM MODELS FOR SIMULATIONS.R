...
png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM DOC REM MDL.png",  width = 1200, height = 900)
plot(gamm_docrem$gam, pages = 1, residuals = T, scheme = 2, rug = T, scale =-1, shift = coef(gamm_docrem$gam)[1], seWithMean = TRUE)
dev.off()

png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM DOC REM DIAG.png",  width = 1200, height = 900)
par(mfrow = c(2,2))
...
png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM CLR TURB MDL.png", width = 1200, height = 900)
plot(gamm_clr_turb$gam, pages = 1, residuals = T, scheme = 2, rug = T, scale =-1, shift = coef(gamm_clr_turb$gam)[1], seWithMean = TRUE)
dev.off()

png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM CLR TURB DIAG.png", width = 1200, height = 900)
par(mfrow = c(2,2))
...
png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM FILT TURB MDL.png",  width = 600, height = 500)
...
png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM FILT TURB DIAG.png",  width = 500, height = 500)
par(mfrow = c(2,2))
...
png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM FILT AL MDL.png",  width = 600, height = 500)
plot(gamm_comb_filt_al$gam, pages = 1, residuals = T, scheme = 2, rug = T, scale =-1, shift = coef(gamm_comb_filt_al$gam)[1], seWithMean = TRUE)
dev.off()

png(filename = "PLOTS/MODELS USING ALL DATA/PCA GAMM FILT AL DIAG.png",  width = 500, height = 500)
par(mfrow = c(2,2))
...
save.image(file = "DATA/SIMULATION_GAMM_MODELS.Rdata")
