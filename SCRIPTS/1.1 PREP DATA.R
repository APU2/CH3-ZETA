...
ts_eg_clr_doc<- tbl(rosdb, "clr_doc")%>%
          ...
          dplyr::filter(obstime > "2017-12-19 00:00:00",
                        obstime < "2017-12-23 00:00:00")%>%
          arrange(obstime)%>%
          mutate(clr_doc = exp(hampel(log(clr_doc), k = 12, t0 = 2)$y))%>%
          transmute(obstime = obstime,
                    signal = "clr_uv_abs",
                    value = clr_doc*1.5)
...
zeta<- tbl(rosdb, "zeta")%>%collect(n = Inf)%>%arrange(obstime)%>%
          ...
          summarise(zetap = mean(zetap, na.rm = TRUE, trim = 0.1),
                    qual_factor = mean(qual_factor))%>%
          ungroup()%>%
          mutate(zetap = hampel(zetap, k = 12, t0 = 2)$y)
...
raw_doc<- tbl(rosdb, "raw_doc")%>%
          ...
          mutate(raw_doc = exp(hampel(log(raw_doc), k = 12, t0 = 2)$y))
...
sigs_rawtime<- tbl(rosdb, "sigs")%>%
          ...
          mutate(megget_inlet_fl = hampel(megget_inlet_fl, k = 12, t0 = 2)$y,
                 gladhouse_inlet_fl = hampel(gladhouse_inlet_fl, k = 12, t0 = 2)$y,
                 sldg_sprntnt_return_fl = hampel(sldg_sprntnt_return_fl, k = 12, t0 = 2)$y,
                 raw_water_inlet_temperature = hampel(raw_water_inlet_temperature, k = 12, t0 = 2)$y,
                 sprntnt_turb = hampel(sprntnt_turb, k = 12, t0 = 2)$y,
                 raw_water_turb = hampel(raw_water_turb, k = 12, t0 = 2)$y,
                 raw_water_ph = hampel(raw_water_ph, k = 12, t0 = 2)$y,
                 raw_col = hampel(raw_col, k = 12, t0 = 2)$y,
...
sigs_coagtime<- tbl(rosdb, "sigs")%>%
          ...
          mutate(coagulated_water_ph = hampel(coagulated_water_ph, k = 12, t0 = 2)$y)
...
sigs_clrtime<- tbl(rosdb, "sigs")%>%
          ...
          group_by(obstime)%>%
          summarise_all(funs(median), na.rm = TRUE, trim = 0.1)%>%
          mutate(daf_saturator_1_lvl = hampel(daf_saturator_1_lvl, k = 12, t0 = 2)$y,
                 daf_saturator_2_lvl = hampel(daf_saturator_2_lvl, k = 12, t0 = 2)$y,
                 daf_saturator_1_prs = hampel(daf_saturator_1_prs, k = 12, t0 = 2)$y,
                 daf_saturator_2_prs = hampel(daf_saturator_2_prs, k = 12, t0 = 2)$y,
                 daf_str_1_outlet_turb = hampel(daf_str_1_outlet_turb, k = 12, t0 = 2)$y,
                 daf_str_2_outlet_turb = hampel(daf_str_2_outlet_turb, k = 12, t0 = 2)$y,
                 daf_str_3_outlet_turb = hampel(daf_str_3_outlet_turb, k = 12, t0 = 2)$y)
...
sigs_filttime<- tbl(rosdb, "sigs")%>%
          ...
          group_by(obstime)%>%
          summarise_all(funs(median), na.rm = TRUE, trim = 0.1)%>%
          mutate(comb_filt_al = hampel(comb_filt_al, k = 12, t0 = 2)$y)%>%
          mutate(comb_filt_turb = hampel(comb_filt_turb, k = 12, t0 = 2)$y)
...
pacl_tank<- sigs%>%
          transmute(obstime = obstime,
                    pacl_tank1 = hampel(alum_storage_tank_2_lvl, k = 12, t0 = 0.3)$y,
                    pacl_tank2 = hampel(alum_storage_tank_1_lvl, k = 12, t0 = 0.3)$y,
...
pacl_pump<- sigs%>%
          ...
          na.omit()%>%
          ungroup()%>%
          mutate(pacl_pumpspeed = hampel(pacl_pumpspeed, k = 12, t0 = 2)$y)
...
daf_turb<- sigs%>%
          ...
          mutate(clr_turb = hampel(clr_turb, k = 12, t0 = 2)$y)
...
flows<- sigs%>%
          ...
          na.omit()%>%
          ungroup()#%>%
          #mutate(flow = hampel(flow, k = 12, t0 = 2)$y,
           #      glad_prop = hampel(glad_prop, k = 12, t0 = 2)$y)
...
# 
# 
# filtsigs<- tbl(rosdb, "filt_sigs")%>%
#           dplyr::filter(obstime > "2016-10-20 00:00:00",
#                         rgf ==7,
#                         inservice == "ONLINE")%>%
#           collect(n = Inf)%>%
#           mutate(obstime = obstime+minutes(45),
#                     obstime = floor_date(obstime, unit = "3 hours"))%>%
#           group_by(obstime, filter)%>%
#           summarise_all(funs(mean), na.rm = TRUE, trim = 0.1)%>%
#           ungroup()


# filtsigslag<-filtsigs%>%ungroup()%>%mutate(obstime = obstime)%>%
#           left_join(.,sigs_imputed%>%dplyr::select(obstime, raw_water_turb, raw_col,raw_uv_abs,clr_uv_abs, zetap, glad_prop, est_sup_ret,sprntnt_turb, temp))%>%
#           mutate( )%>%
#           group_by(filter, runid)%>%
#           mutate(runflow = round(median(fl, na.rm = T)/2)*2,
#                  timeinrun = timeinrun/3600)%>%
#           mutate(fl_m3hr = fl/1000*3600,
#                  vel = fl_m3hr/52.5)%>%
#           group_by(runid,filter)%>%
#           mutate(inst_vol = fl_m3hr*(timeinrun-lag(timeinrun)),
#                  inst_vol = ifelse(is.na(inst_vol)==TRUE, 0, inst_vol),
#                  filt_vol = cumsum(inst_vol),
#                  bv = filt_vol/52.5)%>%
#           ungroup()%>%
#           # mutate(vel_fct = cut(mean_vel, breaks = c(0,2,3.5,99), labels = c("<3 m/hr","3-3.5 m/hr",  ">3.5 m/hr")),
#           #        zeta_fct = cut(mean_zeta, breaks = c(-8,-3,-1, 1,5), labels = c("<-3mV", "-3to-1mV","-1 to 1 mV" ,"> 1mV")),
#           #        bv_fct = round(bv/10)*10)%>%
#           mutate(datasplit = "train",
#                  datasplit = ifelse(obstime>"2017-10-20 00:00:00", "test", datasplit))%>%
#           dplyr::filter(is.na(obstime)==F)%>%
#           dplyr::filter(obstime< "2018-01-21 00:00:00")
# 
# summary(filtsigslag)
# 
# ########
# 
# 
# filtsigs_imputed<-filtsigslag%>%
#           mutate(raw_water_turb = na.locf(raw_water_turb),
#                  raw_col = na.locf(raw_col),
#                  raw_uv_abs = na.locf(raw_uv_abs),
#                  clr_uv_abs = na.locf(clr_uv_abs),
#                  zetap = na.locf(zetap),
#                  glad_prop = na.locf(glad_prop),
#                  est_sup_ret= na.locf(est_sup_ret),
#                  temp = na.locf(temp),
#                  sprntnt_turb = na.locf(sprntnt_turb))
# 
# ########


save.image(file = "DATA/ZetaPaperData.Rdata")
